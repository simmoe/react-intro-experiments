...
function MoveSelection(clientid, from, to) {
    var jqfrom = $("select#" + clientid + "_" + from + " :selected");
    var jqto = $("select#" + clientid + "_" + to);
    ...
    /*$("option", jqto).sort(
		function(a, b) { return a.innerHTML < b.innerHTML ? -1 : 1; }
	).remove().appendTo(jqto);*/
	var useDirect = jqfrom.length <= 10;
...
function MarkAllInSelectionList(clientid) {
	$("select#" + clientid + "_selectedSet").each(function () {
		for (var count = 0; count < this.options.length; count++)
			this.options[count].selected = true;
	});
}

function PersistSelected(clientid) {
    //var jqfrom = $("select#"+clientid + "_totalSet");
    var jqto = $("select#" + clientid + "_selectedSet");

    //var fromhidden = $("input#" + clientid + "_unselectedIds");
    var tohidden = $("input#" + clientid + "_selectedIds");
...
function SelectBoxSort(selectboxJid, doInlineSort) {

    if (doInlineSort) {
        // Indsat nederst i listen op til 10 usorterede linier
        // Søg listen igennem: er [i] < [i - 1] ? Så går man hele vejen tilbage indtil den passer ind i listen
        var options = $("option", selectboxJid); //selectboxJid.options; // bedre med $("option", selectboxJid) ?
        //var inxOfOptionToReplace = options.length - 1;
        //alert("inline: " + inxOfOptionToReplace);
        //$("#gryffe").after('<option value="1643371221" id="gryffe3" title="3i 01" sortint="0">options: ' + options.length + '</option>');
        //alert("start");
        for (inxOfOptionToReplace = Math.max(options.length - 10, 0); inxOfOptionToReplace < options.length; inxOfOptionToReplace++) {
//            if (confirm(inxOfOptionToReplace + "Stop?"))
//                break;
            //$("#gryffe").after('<option value="1643371221" id="gryffe3" title="3i 01" sortint="0">inxOfOptionToReplace: ' + inxOfOptionToReplace + '</option>');

        	var optionToReplace = options[inxOfOptionToReplace]
            var testOptionInt = parseInt($(optionToReplace).attr("sortint"));
            //alert("testoption:" + testOptionInt);
            var newPosition = 0;
            for (i = inxOfOptionToReplace; i > 0; i--) {
                if (testOptionInt > parseInt($(options[i - 1]).attr("sortint"))) {
                    newPosition = i;
                    break;
                }
            }
            ...
            if (newPosition < inxOfOptionToReplace) {
                //alert("move newPosition: " + newPosition + " from " + inxOfOptionToReplace);
                $(options[newPosition]).before(optionToReplace);
                /*
                for (i = inxOfOptionToReplace; i > newPosition; i--) {
                options[i] = options[i - 1].clone();
                alert("moving: " + i);
                }
                options[newPosition] = optionToReplace;
                */
                // Uheldigt at denne må køres hver gang - men opdateres jo ikke når optionen ovenover er blevet flyttet:
                options = $("option", selectboxJid); //selectboxJid.options; // bedre med $("option", selectboxJid) ?
            }
        }
    }
    else {
        $("option", selectboxJid).sort(function(a, b) { return parseInt($(a).attr("sortint")) - parseInt($(b).attr("sortint")); }).remove().appendTo(selectboxJid);  //name //return a.sortint - b.innerHTML ? -1 : 1
    }
...
﻿///<reference path="~/Scripts/jquery-1.8.2.intellisense.js" />
///<reference path="~/scripts/lectio.js" />

var ThumbViewer = (function () {
	...
	function show(event) {
		...
		var $div = $("div#thumbctrl_largeimg");
		if ($div.length == 0) {
		...
		var img = $(evt.target || window.event.srcElement);
		var x = evt.clientX + (window.pageXOffset || document.documentElement.scrollLeft || document.body.scrollLeft);
		var y = evt.clientY + (window.pageYOffset || document.documentElement.scrollTop || document.body.scrollTop);

		// empty the div
		$div.empty();
		$div.css('left', (x - 35) - (x % 35));
		$div.css('top', (y - 130));
		$div.css('z-index', 10000);
		var imgurl = img.attr('src');
		var imgurl2 = img.attr('src2');
		...
		$div.attr('data-lectiocontextcard', lccId);
		$div.attr('imgid', img.attr('id'));

		var editIconHtml = "<img src='/lectio/img/edit2.auto' onclick='ThumbViewer.OnEditClick(event)' style='position: absolute; top: 5px; right: 5px; width:24px; height:24px; background-color: white; border-radius: 10px; padding: 3px;'>";
		var showEditIcon = false;
		$div.append("<span><img src='" + imgurl + "'>" + (showEditIcon ? editIconHtml : "") + "</span>");
		$div.show(100);
	}
...
(function ($) {
	...
	$.fn.cluetip = function (js, options) {
		...
		if (!$cluetip.length) {
			...
			$cluetip.on('mouseenter mouseleave', function (event) {
			...
			****************************************/
			...
			var activate = function (event) {
				...
				***************************************/
					...
					if (tpl > 1) {
						for (var i=1; i < tpl; i++) {
							tipParts[i] = $("<div />").text(tipParts[i].replace(/\r?\n/g, '$BR$')).html().replace(/\$BR\$/g, "<br/>"); //Wrap and insert breaking-space
			...
			var cluetipShow = function (bpY) {
				...
				if (opts.sticky) {
					...
					if (opts.mouseOutClose) {
						$link.off('mouseleave.cluetipMoc');
						$cluetip.off('mouseleave.cluetipMoc');
						if (opts.mouseOutClose === 'both' || opts.mouseOutClose === 'cluetip' || opts.mouseOutClose === true) { // true implies 'cluetip' for backwards compatability
							$cluetip.on('mouseleave.cluetipMoc', mouseOutClose);
						}
						if (opts.mouseOutClose === 'both' || opts.mouseOutClose === 'link') {
							$link.on('mouseleave.cluetipMoc', mouseOutClose);
						}
					}
				}
			...
			if ((/click|toggle/).test(opts.activation)) {
				...
				if (opts.clickThrough) {
					$link.off('click.cluetip', returnFalse);
				}
				else {
					$link.on('click.cluetip', returnFalse);
				}
...
$.noty.themes.defaultTheme = {
    ...
    style   : function() {
        ...
        switch(this.options.type) {
        	case 'warning':
        	case 'alert':
            	this.$bar.css({ backgroundColor: '#fff', borderColor: '#e59100', color: '#826200' });
            	this.$buttons.css({ borderTop: '1px solid #e59100' });
                break;
            case 'error':
            	this.$bar.css({ backgroundColor: '#fff', borderColor: '#B80000', color: '#B80000' });
                this.$message.css({fontWeight: 'normal'});
                this.$buttons.css({ borderTop: '1px solid #B80000' });
                break;
			case 'notification':
            case 'information':
            	this.$bar.css({ backgroundColor: '#fff', borderColor: '#466c80', color: '#466c80' });
            	this.$buttons.css({ borderTop: '1px solid #466c80' });
                break;
            case 'success':
            	this.$bar.css({ backgroundColor: '#fff', borderColor: '#339900', color: 'darkgreen' });
            	this.$buttons.css({ borderTop: '1px solid #339900' });
                break;
            default:
...
;(function($) {
	...
	$.fn.raty.defaults = {
		cancel:			false,
		cancelHint:		'annuller vurdering!',
		cancelOff:		'cancel-off.png',
		cancelOn:		'cancel-on.png',
		cancelPlace:	'left',
		click:			null,
		half:			false,
		hintList:		['Elendig', 'Ringe', 'Middel', 'God', 'Fantastisk'],
		noRatedMsg:		'Ikke vurderet endnu',
		number:			5,
		path:			'img/',
		iconRange:		[],
		readOnly:		false,
 ...
 * https://modernizr.com/download/?-csscolumns-mq-setclasses-cssclassprefix:mzr_ !*/
!function(e,n,t){function r(e,n){return typeof e===n}function o(){var e,n,t,o,s,i,a;for(var l in C)if(C.hasOwnProperty(l)){if(e=[],n=C[l],n.name&&(e.push(n.name.toLowerCase()),n.options&&n.options.aliases&&n.options.aliases.length))for(t=0;t<n.options.aliases.length;t++)e.push(n.options.aliases[t].toLowerCase());for(o=r(n.fn,"function")?n.fn():n.fn,s=0;s<e.length;s++)i=e[s],a=i.split("."),1===a.length?Modernizr[a[0]]=o:(!Modernizr[a[0]]||Modernizr[a[0]]instanceof Boolean||(Modernizr[a[0]]=new Boolean(Modernizr[a[0]])),Modernizr[a[0]][a[1]]=o),g.push((o?"":"no-")+a.join("-"))}}function s(e){var n=S.className,t=Modernizr._config.classPrefix||"";if(b&&(n=n.baseVal),Modernizr._config.enableJSClass){var r=new RegExp("(^|\\s)"+t+"no-js(\\s|$)");n=n.replace(r,"$1"+t+"js$2")}Modernizr._config.enableClasses&&(n+=" "+t+e.join(" "+t),b?S.className.baseVal=n:S.className=n)}function i(){return"function"!=typeof n.createElement?n.createElement(arguments[0]):b?n.createElementNS.call(n,"http://www.w3.org/2000/svg",arguments[0]):n.createElement.apply(n,arguments)}function a(){var e=n.body;return e||(e=i(b?"svg":"body"),e.fake=!0),e}function l(e,t,r,o){var s,l,u,f,c="modernizr",d=i("div"),p=a();if(parseInt(r,10))for(;r--;)u=i("div"),u.id=o?o[r]:c+(r+1),d.appendChild(u);return s=i("style"),s.type="text/css",s.id="s"+c,(p.fake?p:d).appendChild(s),p.appendChild(d),s.styleSheet?s.styleSheet.cssText=e:s.appendChild(n.createTextNode(e)),d.id=c,p.fake&&(p.style.background="",p.style.overflow="hidden",f=S.style.overflow,S.style.overflow="hidden",S.appendChild(p)),l=t(d,e),p.fake?(p.parentNode.removeChild(p),S.style.overflow=f,S.offsetHeight):d.parentNode.removeChild(d),!!l}function u(e,n){return!!~(""+e).indexOf(n)}function f(e){return e.replace(/([a-z])-([a-z])/g,function(e,n,t){return n+t.toUpperCase()}).replace(/^-/,"")}function c(e,n){return function(){return e.apply(n,arguments)}}function d(e,n,t){var o;for(var s in e)if(e[s]in n)return t===!1?e[s]:(o=n[e[s]],r(o,"function")?c(o,t||n):o);return!1}function p(e){return e.replace(/([A-Z])/g,function(e,n){return"-"+n.toLowerCase()}).replace(/^ms-/,"-ms-")}function m(n,r){var o=n.length;if("CSS"in e&&"supports"in e.CSS){for(;o--;)if(e.CSS.supports(p(n[o]),r))return!0;return!1}if("CSSSupportsRule"in e){for(var s=[];o--;)s.push("("+p(n[o])+":"+r+")");return s=s.join(" or "),l("@supports ("+s+") { #modernizr { position: absolute; } }",function(e){return"absolute"==getComputedStyle(e,null).position})}return t}function h(e,n,o,s){function a(){c&&(delete P.style,delete P.modElem)}if(s=r(s,"undefined")?!1:s,!r(o,"undefined")){var l=m(e,o);if(!r(l,"undefined"))return l}for(var c,d,p,h,v,y=["modernizr","tspan","samp"];!P.style&&y.length;)c=!0,P.modElem=i(y.shift()),P.style=P.modElem.style;for(p=e.length,d=0;p>d;d++)if(h=e[d],v=P.style[h],u(h,"-")&&(h=f(h)),P.style[h]!==t){if(s||r(o,"undefined"))return a(),"pfx"==n?h:!0;try{P.style[h]=o}catch(g){}if(P.style[h]!=v)return a(),"pfx"==n?h:!0}return a(),!1}function v(e,n,t,o,s){var i=e.charAt(0).toUpperCase()+e.slice(1),a=(e+" "+k.join(i+" ")+i).split(" ");return r(n,"string")||r(n,"undefined")?h(a,n,o,s):(a=(e+" "+z.join(i+" ")+i).split(" "),d(a,n,t))}function y(e,n,r){return v(e,t,t,n,r)}var g=[],C=[],w={_version:"3.3.1",_config:{classPrefix:"mzr_",enableClasses:!0,enableJSClass:!0,usePrefixes:!0},_q:[],on:function(e,n){var t=this;setTimeout(function(){n(t[e])},0)},addTest:function(e,n,t){C.push({name:e,fn:n,options:t})},addAsyncTest:function(e){C.push({name:null,fn:e})}},Modernizr=function(){};Modernizr.prototype=w,Modernizr=new Modernizr;var S=n.documentElement,b="svg"===S.nodeName.toLowerCase(),_=function(){var n=e.matchMedia||e.msMatchMedia;return n?function(e){var t=n(e);return t&&t.matches||!1}:function(n){var t=!1;return l("@media "+n+" { #modernizr { position: absolute; } }",function(n){t="absolute"==(e.getComputedStyle?e.getComputedStyle(n,null):n.currentStyle).position}),t}}();w.mq=_;var x="Moz O ms Webkit",k=w._config.usePrefixes?x.split(" "):[];w._cssomPrefixes=k;var z=w._config.usePrefixes?x.toLowerCase().split(" "):[];w._domPrefixes=z;var E={elem:i("modernizr")};Modernizr._q.push(function(){delete E.elem});var P={style:E.elem.style};Modernizr._q.unshift(function(){delete P.style}),w.testAllProps=v,w.testAllProps=y,function(){Modernizr.addTest("csscolumns",function(){var e=!1,n=y("columnCount");try{(e=!!n)&&(e=new Boolean(e))}catch(t){}return e});for(var e,n,t=["Width","Span","Fill","Gap","Rule","RuleColor","RuleStyle","RuleWidth","BreakBefore","BreakAfter","BreakInside"],r=0;r<t.length;r++)e=t[r].toLowerCase(),n=y("column"+t[r]),("breakbefore"===e||"breakafter"===e||"breakinside"==e)&&(n=n||y(t[r])),Modernizr.addTest("csscolumns."+e,n)}(),o(),s(g),delete w.addTest,delete w.addAsyncTest;for(var N=0;N<Modernizr._q.length;N++)Modernizr._q[N]();e.Modernizr=Modernizr}(window,document);
